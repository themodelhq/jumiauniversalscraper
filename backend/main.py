...
def _run(jid, urls, fmt, delay):
    ...
    for i,url in enumerate(urls):
        ...
        try: r=scrape_product(url); results.append(r); j["results"].append(r)
        except Exception as e: err={"URL":url,"Error":str(e)}; results.append(err); j["results"].append(err)
        j["completed"]=i+1
        if i<len(urls)-1: time.sleep(delay)
    try: fn=_save(results,fmt,jid); j["download_url"]=f"/download/{fn}"
    except Exception as e: j["error"]=str(e)
    j.update({"status":"done","progress":100,"message":f"Done — {len(results)} products scraped"})
...
async def scrape_file(background_tasks: BackgroundTasks,
    ...
    if not urls: raise HTTPException(400,"No valid URLs found in file")
    jid=uuid.uuid4().hex[:8]; _job(jid,len(urls))
...
def _save(results, fmt, jid):
    ...
    else: p=RESULTS_DIR/f"{jid}_raw.csv"; pd.DataFrame(results).to_csv(p,index=False,encoding="utf-8-sig")
    return p.name
...
async def scrape_file(background_tasks: BackgroundTasks,
                      file: UploadFile=File(...), format: str=Form("bob"), delay: float=Form(1.5)):
    content=await file.read(); ext=Path(file.filename).suffix.lower()
    tmp=RESULTS_DIR/f"up_{uuid.uuid4().hex[:8]}{ext}"; tmp.write_bytes(content)
    try:
        df=pd.read_csv(tmp) if ext==".csv" else pd.read_excel(tmp)
        col=next((c for c in df.columns if "url" in c.lower()),df.columns[0])
        urls=[str(v).strip() for v in df[col].dropna() if str(v).strip().startswith(("http://","https://"))]
    except Exception as e: raise HTTPException(400,f"Cannot parse: {e}")
