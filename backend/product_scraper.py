...
def run_scraper(infile, outfile, delay=2):
    """Process input file and scrape all URLs"""
    ...
    urlcol = next((c for c in df.columns if "url" in c.lower()), None)
    if not urlcol:
        raise ValueError("No URL column found in input file.")

    results = []
    total = len(df[urlcol].dropna())

    for idx, url in enumerate(df[urlcol].dropna()):
        url_str = str(url).strip()
        if not url_str or not url_str.startswith(('http://', 'https://')):
            log_debug(f"⚠️ Skipping invalid URL: {url_str}")
            results.append({"URL": url_str, "Error": "Invalid URL format"})
            continue

        print(f"🔎 [{idx+1}/{total}] Scraping: {url_str}")
    ...
    expected_cols = [
        "URL", "Website", "Product Name", "Brand", "Price", "Currency",
    ...
    success_count = len(results_df[results_df.get('Error', pd.Series()).isna()])
...
if __name__ == "__main__":
    ...
    print("="*60)
    print("🛒 Universal Product Scraper v3.0")
    print("Supports: Amazon, Noon, GSM Arena, and any website")
    print("="*60)
