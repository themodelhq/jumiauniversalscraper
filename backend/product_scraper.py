...
def create_session():
    ...
    # Only retry on connection errors, not timeouts (timeout retries waste time)
...
# (connect_timeout, read_timeout) — connect quickly, give read up to 12s
_TIMEOUT = (6, 12)

# Sites known to respond slowly — give them a longer read window
_SLOW_SITES = {
    'asos.com', 'zara.com', 'hm.com', 'uniqlo.com', 'zalando.com',
    'myntra.com', 'flipkart.com', 'jumia.com', 'konga.com', 'jiji.ng',
    'aliexpress.com', 'ebay.com', 'walmart.com', 'target.com',
}
_SLOW_TIMEOUT = (6, 28)   # longer read for slow CDN/JS-heavy sites

def _get_timeout(url: str) -> tuple:
    """Return appropriate (connect, read) timeout for the given URL."""
    try:
        domain = urlparse(url).netloc.lower().lstrip("www.")
        for slow in _SLOW_SITES:
            if slow in domain:
...
def scrape_amazon_product(url):
    try:
        ...
        resp = session.get(url, timeout=_get_timeout(url))
...
# ============================================================
# GSM ARENA SCRAPER — with bot-bypass
# ============================================================

def _gsmarena_fetch_html(url: str) -> str:
    """
    Fetch a GSMArena page using a multi-layer bypass strategy.

    Layer 1: Full Chrome session — warm up with homepage visit to get
             real cookies, then fetch the product page with Referer set.
    Layer 2: Playwright headless browser (if requests fails).
    Layer 3: Unofficial GSMArena mirror API as last resort.

    Returns raw HTML string or raises on complete failure.
    """
    import time as _t, random as _r

    # ── Layer 1: Warm-up session ──────────────────────────────────────────
    _CHROME_UA = (
        "Mozilla/5.0 (Windows NT 10.0; Win64; x64) "
        "AppleWebKit/537.36 (KHTML, like Gecko) "
        "Chrome/124.0.0.0 Safari/537.36"
    ...
    try:
        # Visit homepage first to acquire real session cookies
        home = session.get("https://www.gsmarena.com/", timeout=_TIMEOUT)
        if home.status_code == 200:
            _t.sleep(_r.uniform(1.5, 3.0))
            # Now fetch the product page with Referer
            session.headers["Referer"] = "https://www.gsmarena.com/"
            session.headers["Sec-Fetch-Site"] = "same-origin"
            resp = session.get(url, timeout=_TIMEOUT)
            if resp.status_code == 200 and len(resp.text) > 5000:
                log_debug(f"✅ GSMArena Layer 1 success for {url}")
                return resp.text
            log_debug(f"⚠️ GSMArena Layer 1 got {resp.status_code}, trying Layer 2")
        else:
            log_debug(f"⚠️ GSMArena homepage returned {home.status_code}, trying Layer 2")
    except Exception as e:
        log_debug(f"⚠️ GSMArena Layer 1 error: {e}, trying Layer 2")
    ...
    try:
        ...
        with sync_playwright() as pw:
            ...
            page.goto("https://www.gsmarena.com/", timeout=20000, wait_until="domcontentloaded")
            page.wait_for_timeout(int(_r.uniform(1500, 2500)))
            ...
            if html and len(html) > 5000:
                log_debug(f"✅ GSMArena Layer 2 (Playwright) success for {url}")
                return html
            log_debug(f"⚠️ GSMArena Layer 2 got short response, trying Layer 3")
    except Exception as e:
        log_debug(f"⚠️ GSMArena Layer 2 error: {e}, trying Layer 3")

    # ── Layer 3: Unofficial GSMArena API mirror ───────────────────────────
    # Extract the phone slug from the URL (e.g. "samsung_galaxy_a07-14066")
    try:
        slug_match = re.search(r'/([a-z0-9_]+-\d+)\.php', url)
        if slug_match:
            slug = slug_match.group(1)
            # Try known public mirror APIs
            api_urls = [
                f"https://gsmarena-api.vercel.app/api/search?query={slug.replace('_', '+').replace('-', '+')}",
                f"https://phone-specs-api.azurewebsites.net/search?query={slug.replace('_', ' ')}",
            ]
            for api_url in api_urls:
                try:
                    r = requests.get(api_url, timeout=_TIMEOUT,
                                     headers={"User-Agent": _CHROME_UA, "Accept": "application/json"})
                    if r.status_code == 200:
                        data = r.json()
                        # Convert API JSON to minimal fake HTML for the parser
                        phone = None
                        if isinstance(data, list) and data:
                            phone = data[0]
                        elif isinstance(data, dict):
                            phone = data.get("phone") or data.get("result") or data
                        if phone:
                            log_debug(f"✅ GSMArena Layer 3 (API) success for {url}")
                            return _gsmarena_api_to_html(phone, url)
                except Exception:
                    continue
    except Exception as e:
        log_debug(f"⚠️ GSMArena Layer 3 error: {e}")

    raise RuntimeError(f"All GSMArena fetch layers failed for {url}")


def _gsmarena_api_to_html(data: dict, url: str) -> str:
    """Convert API JSON response into minimal HTML that the parser can extract."""
    name  = data.get("phone_name") or data.get("name") or data.get("DeviceName") or ""
    brand = data.get("brand") or data.get("Brand") or ""
    img   = data.get("image") or data.get("img") or data.get("thumbnail") or ""
    specs = data.get("specifications") or data.get("specs") or {}
    desc  = ""
    if isinstance(specs, dict):
        desc = " | ".join(f"{k}: {v}" for k, v in specs.items() if v)[:2000]
    elif isinstance(specs, list):
        desc = " | ".join(str(s) for s in specs)[:2000]

    return f"""<html><head>
<title>{name} - Full phone specifications</title>
<meta name="description" content="{name} specs: {desc[:300]}">
</head><body>
<h1 class="specs-phone-name-title">{name}</h1>
<div class="specs-photo-main"><img src="{img}" /></div>
...
def scrape_gsmarena_product(url):
    try:
        # Fetch HTML using bypass layers
        html = _gsmarena_fetch_html(url)
        ...
        if not title:
            for selector in [
                "h1.specs-phone-name-title",   # primary GSMArena selector
                "h1.article-title",
                ".help-box h1", "h1", ".product-name",
            ]:
                elem = soup.select_one(selector)
                if elem:
                    text = clean_text(elem.get_text())
        ...
        if not title:
            t = soup.find("title")
            if t:
                title = clean_text(t.get_text()).replace(" - Full phone specifications", "").strip()
        if not title: log_debug(f"⚠️ No Title found for {url}")

        # Brand — from URL slug first (most reliable)
        brand = product_data.get('brand') or ""
        if not brand:
            ...
            brand_patterns = {
                ...
                'asus': 'Asus', 'lenovo': 'Lenovo', 'honor': 'Honor',
                'tecno': 'Tecno', 'infinix': 'Infinix', 'itel': 'Itel',
                'hisense': 'Hisense', 'lg': 'LG', 'blackberry': 'BlackBerry',
            }
        ...
        if not price:
            for selector in [
                ".help-box .price", ".price", "[itemprop='price']",
                ".specs-phone-price", ".pricebox-price",
            ]:
                elem = soup.select_one(selector)
                if elem:
                    price = clean_text(elem.get_text())
        ...
        # Specification tables — try ALL known GSMArena structures
        specs = {}

        # Structure A: #specs-list table (current GSMArena)
        ...
        # Structure B: table.specs-list (older GSMArena)
        if not specs:
        ...
        # Structure C: div-based .tr with .ttl / .nfo
        if not specs:
        ...
        # Description — meta description is most reliable for GSMArena
        description = product_data.get('description') or ""
        if not description:
        ...
        if not description:
            for selector in ["[itemprop='description']", ".article-body", ".review-body"]:
                elem = soup.select_one(selector)
                if elem:
                    description = clean_text(elem.get_text())
                    break
        if not description and tech_info:
            description = f"{title} specifications: {tech_info[:500]}"

        # Images — GSMArena stores images in .specs-photo-main
        images = product_data.get('images', []) or []
        if not images:
            for selector in [
                ".specs-photo-main img", "div.specs-photo-main img",
                "[itemprop='image']", ".article-img img",
            ]:
                elem = soup.select_one(selector)
                if elem:
        ...
        # Fallback — look for any gsmarena CDN image
        if not images:
            for img in soup.select("img[src]"):
                src = img.get('src', '')
                if 'fdn.gsmarena.com' in src or 'img.gsmarena.com' in src:
                    if src not in images:
                        images.append(src)
...
def extract_product_from_json_ld(product):
    ...
    if 'offers' in product:
        ...
        if isinstance(offers, dict):
            if 'price' in offers and offers['price'] not in (None, "", {}, []):
                data['price'] = str(offers['price'])
            if 'priceCurrency' in offers:
                data['currency'] = str(offers['priceCurrency'])
            if 'availability' in offers:
                data['availability'] = str(offers['availability'])
        elif isinstance(offers, list) and offers:
            first = offers[0]
            if isinstance(first, dict):
                if 'price' in first and first['price'] not in (None, "", {}, []):
                    data['price'] = str(first['price'])
                if 'priceCurrency' in first:
                    data['currency'] = str(first['priceCurrency'])
                if 'availability' in first:
                    data['availability'] = str(first['availability'])
...
def scrape_product(url):
    ...
    # Only fall back to generic scraper if the specialized one truly failed
    # (has an Error key AND no product name) — avoid double-fetching on success
    if result.get("Error") and not result.get("Product Name"):
