    ...
    <div style={{
      ...
      {[
        ...
        <div key={label} style={{
          ...
          <div>
            <div style={{ fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: 26, color: T.textPrimary }}>
              {value}
            </div>
    ...
    <div className="fade-up" style={{
      ...
      <div style={{ padding: "16px 20px", display: "flex", gap: 16, alignItems: "flex-start" }}>
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ fontFamily: "'Syne',sans-serif", fontWeight: 700, fontSize: 15, color: T.textPrimary, marginBottom: 4 }}>
...
// ─── CATEGORY SCRAPER PANEL ──────────────────────────────────────────────────
const CategoryPanel = ({ onJobStart }) => {
  const T = useT();
  ...
  const submit = async () => {
    if (!url.trim()) { setErr("Please enter a category URL"); return; }
    ...
    try {
      const res = await fetch(`${API}/scrape/category`, {
        ...
        body: JSON.stringify({ url: url.trim(), max_products: maxProducts, format: fmt }),
      });
      const data = await res.json();
      if (!res.ok) { setErr(data.detail || "Failed to start category scrape"); return; }
      onJobStart({ job_id: data.job_id, total: data.max_products, format: fmt });
      setUrl("");
    ...
    <Card>
      ...
      <div style={{ display: "flex", gap: 12, alignItems: "center", marginBottom: 20 }}>
        <div style={{ width: 40, height: 40, borderRadius: 10, background: T.orangeGlow, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <Icon d={Icons.grid} size={18} color={T.orange} />
        </div>
        <div>
          <div style={{ fontFamily: "'Syne',sans-serif", fontWeight: 700, fontSize: 16, color: T.textPrimary }}>Category URL Scraper</div>
          <div style={{ fontSize: 12, color: T.textSecond }}>Paste a category or listing page — scrapes all product URLs automatically</div>
        </div>
      </div>
      ...
      <div style={{ padding: "10px 16px", background: T.orangeGlow, borderRadius: 8, border: `1px solid rgba(243,114,24,0.25)`, marginBottom: 20, fontSize: 12, color: T.orange }}>
        ...
        <span style={{ color: T.textSecond }}>
          Any e-commerce category page · <strong style={{ color: T.orange }}>Samsung</strong> (Product Finder API) ·{" "}
          <strong style={{ color: T.orange }}>Fouanistore</strong> (__NEXT_DATA__) · Amazon · Noon · GSMArena · Any website with product listings
        </span>
      </div>

      {/* URL input */}
      <Label>Category / Listing Page URL</Label>
      <div style={{ display: "flex", gap: 10, marginBottom: 20 }}>
        <Input
          value={url} onChange={e => setUrl(e.target.value)}
          placeholder="https://www.samsung.com/africa_en/smartphones/galaxy-a/"
      ...
      <div style={{ display: "flex", gap: 24, marginBottom: 20, flexWrap: "wrap", alignItems: "flex-start" }}>
        ...
        <div>
          ...
          <div style={{ display: "flex", gap: 6 }}>
            {[20, 50, 100, 200].map(n => (
              <Pill key={n} active={maxProducts === n} onClick={() => setMaxProducts(n)}>{n}</Pill>
            ))}
          </div>
        </div>
      </div>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end" }}>
        <OrangeBtn onClick={submit} loading={loading} disabled={loading || !url.trim()}>
          <Icon d={Icons.grid} size={16} color={T.white} />
          Scrape Category
        </OrangeBtn>
      </div>

      {err && (
        <div style={{ marginTop: 14, padding: "10px 14px", background: "rgba(239,68,68,0.1)", border: "1px solid rgba(239,68,68,0.3)", borderRadius: 8, fontSize: 13, color: T.error }}>
          {err}
        </div>
      )}
    </Card>
...
const BrandSearchPanel = ({ onResult, onJobStart }) => {
  ...
  const [loading, setLoading] = useState(false);
  const [err, setErr] = useState("");
  const elapsed = useElapsedTimer(loading && mode === "single");
  ...
  return (
    <Card>
      <div style={{ display: "flex", gap: 12, alignItems: "center", marginBottom: 20 }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontFamily: "'Syne',sans-serif", fontWeight: 700, fontSize: 16, color: T.textPrimary }}>Brand Website Search</div>
          <div style={{ fontSize: 12, color: T.textSecond }}>Search brand official sites by product name — no URL needed</div>
        </div>
        {loading && mode === "single" && (
          <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "6px 14px", borderRadius: 20, background: T.orangeGlow, border: `1px solid rgba(243,114,24,0.3)` }}>
            <Spinner size={14} />
            <span style={{ fontSize: 12, color: T.orange, fontFamily: "'Syne',sans-serif", fontWeight: 700 }}>{elapsed}s</span>
          </div>
        )}
      </div>
      ...
      <div style={{ padding: "10px 16px", background: T.orangeGlow, borderRadius: 8, border: `1px solid rgba(243,114,24,0.25)`, marginBottom: 20, fontSize: 12, color: T.orange }}>
        ...
        <span style={{ color: T.textSecond }}>e.g. <code style={{ color: T.textPrimary }}>Samsung Galaxy A06 Black 128GB SM-A065FZKHAFB</code></span>
      </div>
      ...
      {mode === "single" ? (
        <>
          {loading && (
            <div style={{ marginBottom: 16, padding: "10px 14px", background: T.mid, borderRadius: 8, fontSize: 12, color: T.textSecond, display: "flex", alignItems: "center", gap: 10 }}>
              <Spinner size={14} />
              <span>
                {elapsed < 5 ? "Searching brand websites…" :
                 elapsed < 15 ? "Scraping product page…" :
                 elapsed < 30 ? "Extracting product details…" :
                 elapsed < 45 ? "Running deep search — almost done…" :
                 "Still working — multi-stage brand search in progress…"}
              </span>
              <span style={{ marginLeft: "auto", color: T.orange, fontWeight: 600 }}>{elapsed}s</span>
            </div>
          )}
  ...
  return (
    <Card>
      <div style={{ display: "flex", gap: 12, alignItems: "center", marginBottom: 20 }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontFamily: "'Syne',sans-serif", fontWeight: 700, fontSize: 16, color: T.textPrimary }}>Single URL Scraper</div>
      ...
      {loading && (
        <div style={{ marginBottom: 16, padding: "10px 14px", background: T.mid, borderRadius: 8, fontSize: 12, color: T.textSecond, display: "flex", alignItems: "center", gap: 10 }}>
          <Spinner size={14} />
          <span>
            {elapsed < 5 ? "Connecting to site…" :
             elapsed < 12 ? "Fetching product page…" :
             elapsed < 20 ? "Parsing product data…" :
             elapsed < 30 ? "Almost there — some sites are slow…" :
             "Still working — large/slow site detected…"}
          </span>
          <span style={{ marginLeft: "auto", color: T.orange, fontWeight: 600 }}>{elapsed}s</span>
        </div>
      )}
  ...
  return (
    <Card>
      <div style={{ display: "flex", gap: 12, alignItems: "center", marginBottom: 20 }}>
        ...
        <div>
          <div style={{ fontFamily: "'Syne',sans-serif", fontWeight: 700, fontSize: 16, color: T.textPrimary }}>Batch URL Scraper</div>
...
const FileUploadPanel = ({ onJobStart }) => {
  ...
  const submit = async () => {
    ...
    try {
      ...
      onJobStart({ job_id: data.job_id, total: data.total, format: fmt });
      setFile(null);
    } catch (e) { setErr("Network error — check that the backend is running"); }
    finally { setLoading(false); }
  };
  ...
  return (
    <Card>
      <div style={{ display: "flex", gap: 12, alignItems: "center", marginBottom: 20 }}>
        ...
        <div>
          <div style={{ fontFamily: "'Syne',sans-serif", fontWeight: 700, fontSize: 16, color: T.textPrimary }}>File Upload Scraper</div>
          <div style={{ fontSize: 12, color: T.textSecond }}>Upload CSV/Excel with a URL column</div>
        </div>
      </div>
      {/* Large batch warning */}
      <div style={{ padding: "10px 16px", background: T.orangeGlow, borderRadius: 8, border: `1px solid rgba(243,114,24,0.25)`, marginBottom: 20, fontSize: 12, color: T.orange }}>
        <strong>Tip:</strong> <span style={{ color: T.textSecond }}>For large batches (100+ URLs), keep the tab open and watch the <strong style={{ color: T.orange }}>Job Status</strong> tab for live progress. The job runs in the background — you can download results when it completes.</span>
      </div>
      {/* Drop zone */}
      <div
  ...
  return (
    <Card className="fade-in">
      <div style={{ display: "flex", gap: 12, alignItems: "center", marginBottom: 20 }}>
        ...
        <div>
          <div style={{ fontFamily: "'Syne',sans-serif", fontWeight: 700, fontSize: 16, color: T.textPrimary }}>
...
export default function App() {
  ...
  const tabs = [
    { id: "category", label: "Category URL",  icon: Icons.grid   },
    ...
    { id: "progress", label: "Job Status",    icon: Icons.eye,   hide: !activeJobId },
  ];
      ...
      <div style={{ minHeight: "100vh", background: T.dark, transition: "background 0.25s" }}>
        ...
        <div style={{ maxWidth: 900, margin: "0 auto", padding: "0 32px 80px" }}>
          ...
          <div className="fade-in" key={tab}>
            {tab === "category" && <CategoryPanel onJobStart={handleJobStart} />}
            {tab === "single" && (
              <>
            ...
            {tab === "query" && (
              <>
                <BrandSearchPanel onResult={handleQueryResult} onJobStart={handleJobStart} />
                {queryResult && (
                  <div ref={queryResultsRef} style={{ marginTop: 24 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                      <Label>Result</Label>
                      {queryResult.download && (
                        <a href={`${API}${queryResult.download}`} download>
                          <OrangeBtn small>
                            <Icon d={Icons.download} size={14} color={T.white} />
                            Download {queryResult.format?.toUpperCase()}
                          </OrangeBtn>
                        </a>
                      )}
                    </div>
                    <ResultCard result={queryResult.result} download={queryResult.download} index={0} />
                  </div>
                )}
              </>
            )}
