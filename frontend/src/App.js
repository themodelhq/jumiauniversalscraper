...
// ─── THEME (Jumia orange + dark + white) ────────────────────────────────────
const T = {
  orange:      "#F37218",
  orangeDeep:  "#D95F00",
  orangeLight: "#FFF3E8",
  orangeGlow:  "rgba(243,114,24,0.15)",
  dark:        "#0D0D0D",
  darkCard:    "#161616",
  darkBorder:  "#252525",
  mid:         "#2A2A2A",
  ...
  <style>{`
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    @keyframes borderPulse {
      0%, 100% { border-color: ${T.darkBorder}; }
      50%       { border-color: ${T.orange}; }
    }
...
const OrangeBtn = ({ children, onClick, disabled, style, small, outline, loading }) => (
  <button
    onClick={onClick}
...
// ─── ICONS ──────────────────────────────────────────────────────────────────
const Icon = ({ d, size = 18, color = T.textSecond, style }) => (
...
const Header = () => (
  <header style={{
    ...
    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
      ...
      <div>
        <div style={{ fontFamily: "'Syne',sans-serif", fontWeight: 800, fontSize: 18, color: T.white, lineHeight: 1 }}>
...
const Hero = () => (
  <div style={{
    ...
    <h1 className="fade-up" style={{
      ...
      fontWeight: 800, lineHeight: 1.05, color: T.white,
    ...
    <Card style={{ margin: "0 32px 32px" }}>
      <button onClick={() => setOpen(x => !x)} style={{
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          <Icon d={Icons.zap} size={18} color={T.orange} />
          <span style={{ fontFamily: "'Syne',sans-serif", fontWeight: 700 }}>Deployment Guide</span>
        </div>
        <Icon d={open ? Icons.x : Icons.eye} size={16} color={T.textSecond} />
      </button>
      {open && (
        <div className="fade-in" style={{ marginTop: 20 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
            {[
              ...
              {
                title: "2. Deploy Frontend → Netlify",
                steps: [
                  "Push the /frontend folder to GitHub",
                  "Create new site on netlify.com → Import from Git",
              ...
              <div key={title} style={{ background: T.mid, borderRadius: 10, padding: 20 }}>
                <div style={{ fontFamily: "'Syne',sans-serif", fontWeight: 700, color: T.orange, marginBottom: 12 }}>{title}</div>
                <ol style={{ paddingLeft: 18, display: "flex", flexDirection: "column", gap: 8 }}>
                  {steps.map((s, i) => (
                    <li key={i} style={{ fontSize: 13, color: T.textSecond, lineHeight: 1.5 }}>{s}</li>
                  ))}
                </ol>
              </div>
            ))}
          </div>
          <div style={{ marginTop: 16, padding: "12px 16px", background: T.orangeGlow, borderRadius: 8, border: `1px solid rgba(243,114,24,0.3)` }}>
            <div style={{ fontSize: 12, color: T.orange, fontFamily: "'Syne',sans-serif", fontWeight: 700, marginBottom: 4 }}>⚡ Update API URL</div>
            <div style={{ fontSize: 12, color: T.textSecond }}>
              After deploying the backend, update <code style={{ background: T.mid, padding: "1px 6px", borderRadius: 4 }}>REACT_APP_API_URL</code> in Netlify's environment variables, then redeploy the frontend.
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{ minHeight: "100vh", background: T.dark }}>
        ...
        <footer style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <span style={{ fontFamily: "'Syne',sans-serif", fontWeight: 700, color: T.white }}>
              Scraper<span style={{ color: T.orange }}>Pro</span>
            </span>
          </div>
          ...
          <div style={{ fontSize: 12, color: T.textMuted }}>
            React + FastAPI · Deploy on Netlify & Render
          </div>
        </footer>
      </div>
    </>
